...
import { t } from "./localization.service.ts";

export const printError = (message: any) => {
	console.log(`${chalk.bgRedBright(" ERROR ")} ${chalk.redBright(message)}`);
};

export const printSuccess = (message: any) => {
	console.log(
		`${chalk.bgGreenBright(" SUCCESS ")} ${chalk.greenBright(message)}`
	);
};

export const printInfo = (message: any) => {
	console.log(
		`${chalk.bgYellowBright(" INFO ")} ${chalk.yellowBright(message)}`
	);
};
...
export const printWeather = (data: WeatherData) => {
	...
	const formatTemp = (temp: number) => {
		if (temp > 30) return chalk.redBright(`${Math.round(temp)}°C`);
		if (temp > 20) return chalk.yellowBright(`${Math.round(temp)}°C`);
		if (temp > 10) return chalk.greenBright(`${Math.round(temp)}°C`);
	...
	const weatherContent = [
		`${chalk.bold(currentWeather.description)} • ${chalk.yellow(
			currentWeather.main
		)}`,
		"",
